...
function ContentAproval({ isDarkMode }) {
...
<Card  style={{ width: '45rem' ,  backgroundColor: isDarkMode ? '#061329' : '#fff', color: isDarkMode ? '#fff' : '#000'}}>
        <Card.Header style={{ backgroundColor: isDarkMode ? '#8B8B8E' : '#32314c', color: isDarkMode ? '#fff' : '#fff' , textAlign:'center' }}>  <FaArrowLeft  /><b style={{marginRight:'69%'}}>Confirm your information</b>  </Card.Header>
        <Card.Body>
          <Card.Title > 
...
<Row className="mb-3">
  ...
  <Form.Group as={Col} controlId="formGridResume">
    ...
    <Form.Control type="file" required onChange={handleFileChange}  style={{fontWeight: 'bold',height: '150px',fontSize: '15px',border: '3px dashed #D3D3D7',backgroundColor:'#D3D3D7'}}  />
      ...
      {/* Submit Button */}
      <Button   className="button" required href="/screen" variant="primary" size="sm" style={{ 
        borderRadius: '13px',
        height: '50px',
        ...
        padding: '12px 40px',
           ...
           }}  onChange={ContentAproval.handleJobStatusChange}>
           Start Screening
       </Button>
    </Form>
          </Card.Text>
